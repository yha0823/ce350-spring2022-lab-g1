...
int test_mem1()
{
    ...
    p[0] = mem_alloc(0x20); // 1. allocate the smallest size of memory (32 bytes) 

    if (p[0] != NULL) {
        result |= BIT(0);
    }

    if (mem_dump() == 7) { // 2. after 1, there should be a total of 7 free blocks in the system
        result |= BIT(1);
    }

    p[1] = mem_alloc(0x01); // 3. allocate 1 byte, should be allocated in the size of 0x20 bytes
    if  (((int)p[0] + 0x20) == (int)p[1]) {
        result |= BIT(2);
    }
    
    p[2] = mem_alloc(0x800); // 4. allocate half of 0x1000 
    if (p[2] != NULL ) {
        result |=BIT(3);
    }
    
    mem_dealloc(p[0]); // 5. deallocate p[0] and p[1]. this should cause coalescing to occur all the way up to level 1 and there should be only 1 free block available
    mem_dealloc(p[1]);
    p[0] = NULL;
    p[1] = NULL;
    ...
    p[3] = mem_alloc(0x800); // 6. allocate the first half of 0x1000. this should completely fill RAM1
    
    if ( mem_dump() == 0 ) {
        result |= BIT(5);
    }
    
    mem_dealloc(p[3]); // 7. deallocate the first half of 0x1000, and allocate two quarters of 0x1000 to the first half space. this should also completely fill RAM1
    p[3] = NULL;
    p[0] = mem_alloc(0x400);
    p[1] = mem_alloc(0x400);
    ...
    mem_dealloc(p[0]); // 8. deallocate all the nodes, there should be only one free block available at the root node (level 0)
    mem_dealloc(p[1]);
    mem_dealloc(p[2]);
    p[0] = NULL;
    p[1] = NULL;
...
int test_mem2()
{
    ...
    p[0] = mem2_alloc(0x20); // 1. allocate the smallest size of memory (32 bytes) 

    if (p[0] != NULL) {
        result |= BIT(0);
    }

    if (mem2_dump() == 10) { // 2. after 1, there should be a total of 10 free blocks in the system
        result |= BIT(1);
    }

    p[1] = mem2_alloc(0x01); // 3. allocate 1 byte, should be allocated in the size of 0x20 bytes
    if  (((int)p[0] + 0x20) == (int)p[1]) {
        result |= BIT(2);
    }
    
    p[2] = mem2_alloc(0x4000); // 4. allocate half of 0x8000 
    if (p[2] != NULL ) {
        result |= BIT(3);
    }
    
    mem2_dealloc(p[0]); // 5. deallocate p[0] and p[1]. this should cause coalescing to occur all the way up to level 1 and there should be only 1 free block available
    mem2_dealloc(p[1]);
    p[0] = NULL;
    p[1] = NULL;
    ...
    p[3] = mem2_alloc(0x4000); // 6. allocate the first half of 0x8000. this should completely fill RAM2
    
    if ( mem2_dump() == 0 ) {
        result |= BIT(5);
    }
    
    mem2_dealloc(p[3]); // 7. deallocate the first half of 0x8000, and allocate two quarters of 0x8000 to the first half space. this should also completely fill RAM2
    p[3] = NULL;
    ...
    mem2_dealloc(p[0]); // 8. deallocate all the nodes, there should be only one free block available at the root node (level 0)
    mem2_dealloc(p[1]);
    mem2_dealloc(p[2]);
    p[0] = NULL;
    p[1] = NULL;
