...
int test_mem1()
{
    ...
    p[0] = mem_alloc(0x80); // 1. Allocate 0x80 bytes and two 0x400 bytes of the memory 
    p[1] = mem_alloc(0x400);
    p[2] = mem_alloc(0x400); // 128, 256, 512, 1024 
    if (mem_dump() == 4) {
        result |= BIT(0);
    }

    p[3] = mem_alloc(0x800); // 2. Try to allocate 0x800 bytes, no free blocks available for the requested size, so cannot allocate

    if (p[3] == NULL) {
        result |= BIT(1);
    }

    p[4] = mem_alloc(0x580); // 3. Attempt to allocate the rest of the memory, then allocate the rest, it is full after this
    p[3] = mem_alloc(0x200);
    if  ((p[4] == NULL) && (mem_dump() == 3)) {
        result |= BIT(2);
    }
    
    mem_dealloc(p[0]); // 4. deallocate everything and the memory will go back to lvl 0, then allocate half of the memory
    mem_dealloc(p[1]);
    mem_dealloc(p[2]);
    mem_dealloc(p[3]);
    p[0] = NULL;
    ...
    mem_dealloc(p[0]); // 5. deallocate first half of 0x1000, allocate 3 blocks of 0x400 bytes, then allocate the rest by 0x200 each. the memory is full after this 
    p[0] = NULL;
    p[0] = mem_alloc(0x400);
    p[1] = mem_alloc(0x400);
    p[2] = mem_alloc(0x400);
    ...
    p[5] = mem_alloc(0x1); // 6. the memory is full, so cannot allocate anymore
    
    if ( p[5] == NULL ) {
        result |= BIT(5);
    }
    
    mem_dealloc(p[0]); // 7. deallocate p[0] and p[1]
    mem_dealloc(p[1]);
    p[0] = NULL;
    p[1] = NULL;
    
    if ( mem_dump() == 1 ) { // memory should be full after 7
        result |= BIT(6);
    }

    mem_dealloc(p[2]); // 8. deallocate the rest of the memory
    mem_dealloc(p[3]);
    mem_dealloc(p[4]);
    p[2] = NULL;
    p[3] = NULL;
...
int test_mem2(void) {
    ...
    p[0] = mem2_alloc(0x4000); // 1. allocate two 0x4000 bytes of the memory. the memory is full after this
    ...
    p[2] = mem2_alloc(0x01); // 2. attempt to allocate 0x01 bytes. the memory is full, so the allocation is unsuccessful

    if (p[2] == NULL) {
        result |= BIT(1);
    }

    mem2_dealloc(p[0]); // 3. deallocate everything. there is a free node at the root of the binary tree at lvl 0
    mem2_dealloc(p[1]);
    p[0] = NULL;
    p[1] = NULL;
    ...
    p[0] = mem2_alloc(0x2000); // 4. allocate four 0x2000 bytes. the memory is full after this 
    p[1] = mem2_alloc(0x2000);
    p[2] = mem2_alloc(0x2000);
    p[3] = mem2_alloc(0x2000);

    if (mem2_dump() == 0){
        result |= BIT(3);
    }
    
    mem2_dealloc(p[0]); // 5. deallocate p[0], then allocate 0x1000 bytes to p[0], there is one free block in the memory
    p[0] = NULL;
    ...
    mem2_dealloc(p[0]); // 6. deallocate all the memory. there is one free block in the root level of the binary tree
    mem2_dealloc(p[1]);
    mem2_dealloc(p[2]);
    mem2_dealloc(p[3]);
    p[0] = NULL;
    ...
    p[0] = mem2_alloc(0x69); // 7. allocate p[0] and p[1] with 0x69 bytes
    p[1] = mem2_alloc(0x69);
    ...
    p[2] = mem2_alloc(0x69); // 8. allocate p[2] and p[3] with 0x69 bytes
    p[3] = mem2_alloc(0x69);
