...
void task_kcd(void)
{
    ...
    int number_of_chars_to_remove = 1;
    ...
    for(int i = starting_index; i < valid_tsks_found; i++) {
        ...
        if (res == RTX_OK) {
            ...
            if (starting_index == 1) {
                ...
                sprintf((char *)temp + MSG_HDR_SIZE, "%s%d %d\r\n", (char *)full_buffer + MSG_HDR_SIZE, all_tasks_buffer[i], task_info.state);
            } else {
                ...
                sprintf((char *)temp + MSG_HDR_SIZE, "%s%d %d %d\r\n", (char *)full_buffer + MSG_HDR_SIZE, all_tasks_buffer[i], task_info.state, free_space);
            }
